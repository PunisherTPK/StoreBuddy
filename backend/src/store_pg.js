...
const dbConfig = {
  ...
  database: process.env.PGDATABASE || process.env.POSTGRES_DB || "storebuddy",
  max: 10
};
...
async function ensureDatabase() {
  const adminConfig = {
    ...dbConfig,
    database: process.env.PGADMIN_DB || process.env.POSTGRES_ADMIN_DB || "postgres"
  };
  const client = new Client(adminConfig);

  try {
    await client.connect();
    const { rows } = await client.query("SELECT 1 FROM pg_database WHERE datname = $1", [dbConfig.database]);
